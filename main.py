...
# takes the coordinates of a specific piece and returns a notation
def coords_to_notation(coords):
    return f'{chr(97 + coords[0])}{8 - coords[1]}'


# takes the notation and returns a coordinate
...
# Start of "No touch"
# functions that draws the board and gives functionality
...
def move_piece(board, target, kings, origin, destination, captures, promotion):
    ...
    if transcript[-2] != 'O':
        if target.name != 'pawn':
            ...
            transcript += coords_to_notation(origin)[0]
        transcript += f'x{coords_to_notation(destination)} ' if board[destination[1]][
